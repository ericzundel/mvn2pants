...
class RunCompilePrepCommand(Task):

  def execute(self):
    targets = self.context.targets(postorder=True, predicate=lambda t: isinstance(t, CompilePrepCommand))
    Cmdline = namedtuple('Cmdline', ['cmdline', 'environ'])

    def make_cmdline(target):
      executable = target.payload.get_field_value('executable')
      args = target.payload.get_field_value('args', [])
      prep_environ = target.payload.get_field_value('environ')
      cmdline = [executable]
      cmdline.extend(args)
      return Cmdline(cmdline=tuple(cmdline), environ=prep_environ)

    cmdlines = [make_cmdline(target) for target in targets]

    if not cmdlines:
      return

    with self.context.new_workunit(name='compile_prep_command', labels=[WorkUnitLabel.PREP]) as workunit:
      ...
      for item in cmdlines:
        ...
        if not cmdline in completed_cmdlines:
          ...
          try:
            ...
            raise TaskError('RunCompilePrepCommand failed to execute {cmdline}: {error}'.format(
          ...
          if process.returncode:
            raise TaskError('RunCompilePrepCommand failed to run {cmdline}'.format(cmdline=cmdline))
